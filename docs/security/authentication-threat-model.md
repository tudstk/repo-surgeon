...
This milestone intentionally has no OAuth exchange, credentials, authenticated
session issuance, or public clone path. It does persist the digest-only session and
one-time OAuth transaction foundation described above, but the current checkout does
not yet authenticate users.
