...
| `link-inside` | A symlink resolves to `src/main.py` inside the canonical root. | Permit the resolved regular file or deny all symlinks by documented policy; never escape the root. |
